"""
Usage:
  pytest tests/test_phase1c.py

Purpose:
  Validate LLM batching, parsing, and merge logic with mocked clients.
...
def test_empty_events_validation():
    ...
    assert (
        "No events provided" in result["error_message"]
...
def test_chunking_by_event_count():
    ...
    events = [
        {"source_file": "test.jsonl", "record_index": i, "raw_event": {"id": i}}
        for i in range(75)
    ]
    ...
    assert (
        len(chunks[0]) == MAX_EVENTS_PER_BATCH
    ), f"First chunk should have {MAX_EVENTS_PER_BATCH} events"
...
def test_chunking_by_character_limit():
    ...
    events = [
        {"source_file": "test.jsonl", "record_index": i, "raw_event": large_event}
        for i in range(10)
    ]
    ...
    for chunk in chunks:
        ...
        ), f"Chunk exceeds character limit: {char_count} > {MAX_PROMPT_CHARS}"
...
def test_salvage_json_with_text():
    ...
    text_with_json = (
        'Here is the analysis: {"status": "success", "findings": []} and some more text'
    )
...
def test_merge_multiple_successful_batches():
    ...
    ), f"Confidence should be average (0.7+0.9)/2=0.8, got {merged['confidence']}"
...
def test_merge_with_partial_failure():
    ...
    ), f"Status should degrade to timeout, got {merged['status']}"
    assert merged["error_message"] == "LLM request timed out"
    assert (
        len(merged["findings"]) == 1
    ), "Should preserve findings from successful batch"
...
def test_retry_logic_mock():
    ...
    events = [
        {"source_file": "test.jsonl", "record_index": 0, "raw_event": {"EventID": 4688}}
    ]
