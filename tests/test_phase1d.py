...
from src.report import generate_report, generate_error_report, SEVERITY_ORDER
from src.schemas import AnalysisOutput, Finding, Hypothesis, Evidence
...
def test_success_report_structure():
    ...
    analysis = AnalysisOutput(
        ...
        hypotheses=[
            Hypothesis(description="Test hypothesis", confidence=0.75)
        ],
    ...
    assert "test.jsonl:0 | powershell.exe" in report
...
def test_error_report_with_partial_findings():
    ...
    assert "test.jsonl:5 | suspicious login" in report
    ...
    analysis = AnalysisOutput(
        ...
        findings=[
            ...
            Finding(
                ...
                severity="critical",
                evidence=[Evidence(source_file="test.jsonl", record_index=1, excerpt="test")]
            ),
            Finding(
                title="Medium Finding",
                summary="Medium severity",
                severity="medium",
                evidence=[Evidence(source_file="test.jsonl", record_index=2, excerpt="test")]
            ),
        ],
        hypotheses=[],
        indicators_of_compromise=[],
...
def test_confidence_formatting():
    ...
    assert "Overall Confidence: 0.56" in report, "Overall confidence not formatted correctly"
    
    # Check hypothesis confidence formatting
    assert "(confidence: 0.12)" in report, "Hypothesis confidence not formatted correctly"
    assert "(confidence: 0.99)" in report, "Hypothesis confidence not formatted correctly"
...
def test_evidence_multiple_items():
    ...
    analysis = AnalysisOutput(
        ...
        findings=[
            Finding(
                ...
                severity="high",
                evidence=[
                    Evidence(source_file="file1.jsonl", record_index=0, event_id="4688", excerpt="cmd.exe"),
                    Evidence(source_file="file1.jsonl", record_index=5, event_id="4688", excerpt="powershell.exe"),
                    Evidence(source_file="file2.jsonl", record_index=10, event_id="4624", excerpt="login attempt"),
                ]
            )
        ],
        hypotheses=[],
    ...
    assert "file1.jsonl:0 | cmd.exe" in report
    assert "file1.jsonl:5 | powershell.exe" in report
    assert "file2.jsonl:10 | login attempt" in report
    
    print("✓ Multiple evidence items displayed correctly")
